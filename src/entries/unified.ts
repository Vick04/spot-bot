// ─────────────────────────────────────────────
// src/entries/unified.ts
// Unified live trading bot + API server
// Single process running both live trading engine and REST/WebSocket API
// Run with: npm run unified (or via Docker)
// ─────────────────────────────────────────────
...
async function initializeManager(): Promise<{ manager: TradingManager; provider: BinanceWebSocketProvider }> {
  ...
  console.log("[Unified] Config loaded:", config);

  // Load watchlist from database
  const watchlist = await loadWatchlist();
  if (watchlist.length === 0) {
    console.warn("[Unified] Watchlist is empty — populate the watchlist table to start trading");
  }

  const symbols = watchlist.map((w) => w.symbol);
  console.log("[Unified] Symbols:", symbols.join(", "));
  ...
  if (openPosition) {
    console.warn(
      `[Unified] WARNING: recovered orphan position ` +
  ...
  console.log("[Unified] Loading warmup candles...");
  for (const symbol of symbols) {
    try {
      ...
      console.error(`[Unified] Failed to warmup ${symbol}:`, (e as Error).message);
    }
  }
  ...
  newProvider.onCandle(async (symbol, candle) => {
    try {
      ...
      console.error(`[Unified] Error processing candle:`, (e as Error).message);
    }
  });

  // Listen to manager events and broadcast to WebSocket clients
  ...
  newManager.on("error", (event: any) => {
    console.error(`[Unified] Manager error:`, event.error);
    broadcastEvent({ type: "error", error: event.error });
  });

  // Start manager and provider
  console.log("[Unified] Starting manager and provider...");
  newManager.start();

  try {
    await newProvider.start();
  } catch (e) {
    console.error("[Unified] Provider failed to start:", (e as Error).message);
    newManager.stop();
    throw e;
  }
  ...
  console.log("[Unified] ✅ Manager and provider started");
...
async function loadConfigFromDatabase() {
  try {
    ...
    console.error("[Unified] Failed to load config from DB:", (e as Error).message);
    return {
      UP_MAX_STREAK: 1,
      DAILY_MAX_TRADES: 2,
...
async function loadWatchlist() {
  try {
    ...
    console.error("[Unified] Failed to load watchlist:", (e as Error).message);
    return [];
  }
}
...
app.get("/api/watchlist", async (_req, res) => {
  try {
    ...
    try {
      ...
      console.log(`[Unified] GET /watchlist: loaded ${rows.length} symbols`);
    } catch (e) {
      console.error(`[Unified] GET /watchlist: DB query failed —`, (e as Error).message);
      throw e;
    }

    const lastPrices = manager?.lastPrices ?? new Map();
    console.log(`[Unified] GET /watchlist: ${lastPrices.size} prices available`);
    ...
    console.error(`[Unified] GET /watchlist error:`, err);
...
app.post("/api/watchlist", async (req, res) => {
  try {
    ...
    if (manager?.isRunning) {
      console.log(`[Unified] Watchlist updated: ${symbols.join(", ")}`);
...
app.patch("/api/watchlist/:symbol/params", async (req, res) => {
  try {
    ...
    console.log(`[Unified] Updated params for ${symbol}:`, params);
...
app.get("/api/status", async (_req, res) => {
  try {
    ...
    console.log(`[Unified] GET /status: fetching current state`);
    ...
    try {
      ...
      console.log(`[Unified] GET /status: loaded ${allClosed.length} closed trades`);
    } catch (e) {
      console.error(`[Unified] GET /status: failed to load trades —`, (e as Error).message);
      throw e;
    }
    ...
    console.log(
      `[Unified] GET /status: running=${isRunning}, balance=$${manager.balance.toFixed(2)}, ` +
    ...
    const err = (e as Error).message;
    console.error(`[Unified] GET /status error:`, err);
    res.status(500).json({ error: err });
  }
});
...
app.put("/api/config", async (req, res) => {
  try {
    ...
    console.log("[Unified] PUT /config received:", req.body);

    // Write all updates to database
    for (const key of keys) {
      if (key in req.body) {
        const val = String(req.body[key]);
        console.log(`[Unified] Upserting ${key} = ${val}`);
        ...
        console.log(`[Unified] Upserted: ${key} => ${result.value}`);
      }
    }

    console.log("[Unified] All updates written to DB");

    // Reload manager config if running
    if (manager?.isRunning) {
      console.log("[Unified] Reloading manager config...");
      const config = await loadConfigFromDatabase();
      ...
      console.log(`[Unified] Manager config reloaded`);
    }
    ...
    console.error("[Unified] PUT /config error:", e);
    res.status(500).json({ error: String(e) });
  }
});
...
wss.on("connection", (ws: WebSocket) => {
  wsClients.add(ws);
  console.log(`[Unified] ✅ WebSocket client connected (${wsClients.size} total)`);

  // Send initial state
  const sendInitialState = async () => {
    try {
      console.log(`[Unified] Preparing initial state...`);
      ...
      try {
        ...
        console.error(`[Unified] Failed to load watchlist from DB:`, (e as Error).message);
        throw e;
      }

      const lastPrices = manager?.lastPrices ?? new Map();
      console.log(`[Unified] Loaded ${watchlist.length} symbols, ${lastPrices.size} prices available`);
      ...
      try {
        ...
        console.log(`[Unified] ✅ Sent initial state (${watchlistData.length} symbols)`);
      } catch (e) {
        console.error(`[Unified] Failed to send initial state:`, (e as Error).message);
        throw e;
      }
    } catch (e) {
      const err = (e as Error).message;
      console.error(`[Unified] Error in sendInitialState:`, err);
      try {
        ws.close(1011, `Error: ${err}`);
      } catch (closeErr) {
        console.error(`[Unified] Failed to close WS connection:`, (closeErr as Error).message);
      }
    }
  };

  sendInitialState().catch((e) => {
    console.error(`[Unified] Unhandled error in sendInitialState:`, (e as Error).message);
    try {
      ws.close(1011, "Internal server error");
    } catch (closeErr) {
      console.error(`[Unified] Failed to close connection:`, (closeErr as Error).message);
    }
  });

  ws.on("close", () => {
    wsClients.delete(ws);
    console.log(`[Unified] ❌ Client disconnected (${wsClients.size} remaining)`);
  });

  ws.on("error", (e) => {
    console.error(`[Unified] ❌ Client error:`, (e as Error).message);
  });
});
...
server.listen(PORT, async () => {
  console.log(`[Unified] 🤖 Unified trading bot + API server starting...`);
  console.log(`[Unified] Server listening on http://localhost:${PORT}`);
  console.log(`[Unified] WebSocket: ws://localhost:${PORT}`);
  console.log(`[Unified] REST Endpoints:`);
  console.log(`[Unified]   GET  /api/watchlist      — list of active symbols`);
  console.log(`[Unified]   POST /api/watchlist      — update watchlist`);
  console.log(`[Unified]   GET  /api/status         — live bot status`);
  console.log(`[Unified]   GET  /api/trades         — closed trade history`);
  console.log(`[Unified]   GET  /api/config         — read bot config`);
  console.log(`[Unified]   PUT  /api/config         — write bot config`);
  console.log(`[Unified]   GET  /api/health         — health check`);

  // Initialize manager and provider on startup
  try {
    await initializeManager();
    console.log(`[Unified] ✅ Manager initialized and running`);
  } catch (e) {
    console.error(`[Unified] ❌ Failed to initialize manager:`, (e as Error).message);
    console.log(`[Unified] Continuing in API-only mode (live trading disabled)`);
  }
});
...
process.on("SIGINT", async () => {
  console.log("\n[Unified] SIGINT received, shutting down...");
  ...
  if (provider) {
    try {
      ...
      console.error(`[Unified] Failed to stop provider:`, (e as Error).message);
    }
  }
...
process.on("uncaughtException", (err) => {
  console.error("[Unified] ❌ Uncaught exception:", err);
  process.exit(1);
});

process.on("unhandledRejection", (reason) => {
  console.error("[Unified] ❌ Unhandled rejection:", reason);
  process.exit(1);
});
